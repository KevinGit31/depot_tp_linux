...
# ps égale Poste
PACKAGES_LIST="gnupg gnupg2 gnupg1 openjdk-11-jdk ufw jenkins cron"
...
ps_create_partition_ext4(){
    ...
    echo "format partition created"
    sudo mkfs.ext4 $PARTITION_EXT4
    echo "create new directory where the new drive will mount"
    mkdir $STORAGE
    mount $PARTITION_EXT4 $STORAGE
}
